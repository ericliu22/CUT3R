...
def _process_one_image(path: str, idx: int, size: int, square_ok: bool, verbose: bool):
    """Load, resize, crop, and normalize a single image."""
    # open + EXIF fix + RGB
    img = exif_transpose(PIL.Image.open(path)).convert("RGB")
    W1, H1 = img.size

    # resize logic (same as original)
    if size == 224:
    ...
    if verbose:
        print(
            f" - adding {os.path.basename(path)} with resolution "
            f"{W1}x{H1} --> {W2}x{H2}"
        )

    # normalize + pack metadata
    return dict(
...
def load_images(folder_or_list, size, square_ok=False, verbose=True):
    ...
    if isinstance(folder_or_list, str):
        ...
        image_paths = [
            ...
            for name in names
            if name.lower().endswith(supported_images_extensions)
        ]
        ...
        root = ""
        # entries may already be full paths or relative paths
        image_paths = [
            path for path in folder_or_list
    ...
    # -------------------------
    if not image_paths:
        raise AssertionError("no images found at " + root)
    ...
    tasks = [
        (path, idx, size, square_ok, verbose)
